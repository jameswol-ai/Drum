...
# Game‑style UI + user login + rhythm + quests
# =============================
...
# Ensure quests are initialised (in case they weren't)
init_quests(mem)
...
if page == "Command Center":
    ...
    if st.session_state.active_building:
        ...
        # Show the most recent rhythm for this building (if any)
        ...
        for i, b_dict in enumerate(reversed(mem["buildings"])):
            ...
            with st.expander(f"{building.name} – Score {building.score}"):
                ...
                if i < len(mem["rhythms"]):
                    # Show corresponding rhythm (reverse order)
                    rhy = mem["rhythms"][-i-1]
                    show_rhythm(rhy)
    else:
